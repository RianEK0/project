...
const auditActions = [
  ...
  'performance.employee_review.submitted',
  'performance.manager_review.submitted',
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <CardDescription className="mt-2 max-w-2xl text-base leading-7">
                Perubahan penting dari employee, organization, leave workflow, recruitment, performance, payroll,
                IT asset, dan notification center tetap terekam, sekarang dengan tampilan yang lebih tenang dibaca.
              </CardDescription>
            </div>
          </div>
      ...
      <Card>
        <CardHeader className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <CardDescription>
              Filter event berdasarkan action apa pun yang tertangkap backend, termasuk audit otomatis perubahan record.
            </CardDescription>
          </div>
          <div className="w-full md:max-w-sm">
            <input
              ...
              placeholder="Cari action, mis. record.employee.updated"
              value={action}
          ...
          {!logsQuery.isLoading && logsQuery.data?.items.map((log) => (
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs text-app-muted-foreground">
                ...
                <span>Browser: {log.browser ?? 'Unknown'}</span>
                <span>Subject: {labelizeSubject(log.auditable_type)} #{log.auditable_id}</span>
              </div>

              {log.user_agent ? (
                <div className="mt-3 rounded-[18px] border border-app-border bg-app-background/55 px-4 py-3 text-xs leading-6 text-app-muted-foreground">
...
function labelizeSubject(value: string) {
