...
export function FinancePlaceholderPage({
  ...
  highlights,
  relatedLinks = [],
  badgeLabel = 'Finance / Accounting Foundation',
        ...
        <SurfaceCard className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h3 className="text-xl font-semibold">Lanjut ke jalur finance</h3>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {relatedLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-2xl border border-slate-200/80 px-4 py-3 text-sm font-medium transition hover:border-amber-300 hover:bg-amber-50/70 dark:border-slate-800 dark:hover:border-amber-900 dark:hover:bg-slate-900"
              >
                {link.label}
              </Link>
            ))}
