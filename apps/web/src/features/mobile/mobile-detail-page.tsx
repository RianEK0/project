...
import { BrowserCapabilityPanel } from './browser-capability-panel';
import { type MobileCapabilityItem } from './mobile-capability-catalog';
import { MobileApiPreviewPanel } from './mobile-api-preview-panel';
import { ThemeModePanel } from './theme-mode-panel';
import { TouchSurfacePreview } from './touch-surface-preview';

export function MobileDetailPage({ capability }: { capability: MobileCapabilityItem }) {
  return (
    <div className="space-y-4">
      <SurfaceCard tone="accent" className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <StatusBadge tone="success">Browser Checks Live</StatusBadge>
      ...
      <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
        <SurfaceCard className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h3 className="text-xl font-semibold">Yang dibuka lebih cepat oleh capability ini</h3>
            </div>
        ...
        {capability.apiPreview ? (
          <MobileApiPreviewPanel capabilitySlug={capability.slug} />
        ) : (
          <BrowserCapabilityPanel capabilitySlug={capability.slug} />
        )}
      </div>

      {capability.apiPreview ? <BrowserCapabilityPanel capabilitySlug={capability.slug} /> : null}
      {capability.slug === 'dark-mode' ? <ThemeModePanel /> : null}
      {capability.touchSurface ? <TouchSurfacePreview capabilitySlug={capability.slug} /> : null}

      <SurfaceCard className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-sm uppercase tracking-[0.22em] text-muted">Rute terkait</p>
            <h3 className="text-xl font-semibold">Lanjut ke jalur mobile yang terhubung</h3>
          </div>
