...
export function PortalPlaceholderPage({
  ...
  highlights,
  relatedLinks = [],
  badgeLabel = 'Customer Portal Foundation',
        ...
        <SurfaceCard className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h3 className="text-xl font-semibold">Lanjut ke jalur portal</h3>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {relatedLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-2xl border border-slate-200/80 px-4 py-3 text-sm font-medium transition hover:border-emerald-300 hover:bg-emerald-50/70 dark:border-slate-800 dark:hover:border-emerald-900 dark:hover:bg-slate-900"
              >
                {link.label}
              </Link>
            ))}
