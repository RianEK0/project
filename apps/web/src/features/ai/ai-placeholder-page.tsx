...
export function AiPlaceholderPage({
  ...
  highlights,
  relatedLinks = [],
  badgeLabel = 'AI / Intelligence Foundation',
}: AiPlaceholderPageProps) {
  return (
        ...
        <SurfaceCard className="space-y-4 rounded-[30px]">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h3 className="text-xl font-semibold">Lanjut ke jalur lain</h3>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {relatedLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-2xl border border-slate-200/80 px-4 py-3 text-sm font-medium transition hover:border-cyan-300 hover:bg-cyan-50/70 dark:border-slate-800 dark:hover:border-cyan-900 dark:hover:bg-slate-900"
              >
                {link.label}
              </Link>
            ))}
